...
async function handleApi(request,url,env,ctx){
 ...
 if(url.pathname==="/api/debug/access"&&request.method==="GET")return json({
  accessContext:Boolean(ctx.access),
  accessEmailHeader:Boolean(request.headers.get("cf-access-authenticated-user-email")),
  authorizationCookie:(request.headers.get("cookie")||"").includes("CF_Authorization="),
  teamDomainConfigured:Boolean(String(env.ACCESS_TEAM_DOMAIN||"").trim()),
  identityResolved:Boolean(identity?.email),
  identitySource:identity?.source||null
 });
 if(!identity?.email)return json({error:"Sign-in required."},401);
...
async function getIdentity(request,ctx,env){
 if(ctx.access){
  ...
  if(identity?.email)return {email:String(identity.email).trim().toLowerCase(),name:String(identity.name||"").trim(),source:"ctx.access"};
 }
 const accessEmail=request.headers.get("cf-access-authenticated-user-email");
 if(accessEmail)return {email:String(accessEmail).trim().toLowerCase(),name:"",source:"access-header"};

 const cookie=request.headers.get("cookie")||"";
 const teamDomain=String(env.ACCESS_TEAM_DOMAIN||"").trim().replace(/^https?:\/\//i,"").replace(/\/+$/,"");
 if(teamDomain&&cookie.includes("CF_Authorization=")){
  try{
   const res=await fetch(`https://${teamDomain}/cdn-cgi/access/get-identity`,{headers:{cookie}});
   if(res.ok){
    const identity=await res.json();
    if(identity?.email)return {email:String(identity.email).trim().toLowerCase(),name:String(identity.name||"").trim(),source:"identity-endpoint"};
   }
  }catch(err){console.error("Access identity lookup failed",err)}
 }
 return null;
}
